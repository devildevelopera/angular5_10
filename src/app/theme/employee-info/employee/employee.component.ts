  ...
  phone1: any;
   ...
   UpdateEmployeeById(id){
    Swal.fire({
      ...
      if (res.value) {
        this.ngxService.start();
        this.service.GetEmployeeById(id).then(response => {
          this.ngxService.stop();
          if(response.id!=undefined)
          {
            this.route.navigate(['/employee/edit',response.id]);
            sessionStorage.setItem('key',response.id);
          }
          else{
            this.addNotify({title:'Error', msg: response.title,  type:'error'});
          }
           // this.enableEditMode=true;
           // this.modal=response;
      })
   ...
   getEmployeesList(): any {
    this.ngxService.start(); 
     this.service.getEmployeeList().then(response=>{
        this.ngxService.stop();
       if(response.length>0)
       {
          this.addNotify({ title: 'Success', msg: 'Employees found successfully', type: 'success' });
         this.employeeList= response;
       }
       else
       {
   ...
   constructor(private service:EmployeeInfoService,public route:Router,public fb:FormBuilder,
   ...
   deleteEmployeeById(id){
    Swal.fire({
      ...
      if (res.value) {
        this.ngxService.start();
        this.service.deleteEmployeeById(id).then(res=>{
          this.ngxService.stop();
          if (res.status == 200) {
