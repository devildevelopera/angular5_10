...
export class PermissionAddComponent implements OnInit {
  ...
  constructor(private service:PermissionInfoService,private servicePNotify: NotificationsService,
  ...
  SubmitAccessLevelDetails(formData):any{
    if (this.AccessLevelForm.valid) {
     ...
     this.service.SubmitAccessLevelDetails(this.AccessLevelForm.value).then(res=>{
      this.ngxService.stop();
      if(res.status==undefined && res.id!=undefined)
      {
        this.AccessLevelForm.reset();
      ...
      this.addNotify({title:'Error', msg: err.detail,  type:'error',subType: 'warn'});
     })
